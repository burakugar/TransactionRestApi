...
This service service has 7 endpoints.
...
For add transaction to DB, use the POST method and fill the below body:
...
For getting a one special transaction, give the id name to below url:
...
It will return the list of transaction that given actor's payments.
...
It will return the list of transaction that given type has.
For update a transaction, use below endpoint with the transaction id you want to change.
...
For deleting a transaction, use below endpoint:
...
Exceptions:
If no transaction found on db, below exception is thrown.
...
If the size of the String body's are longer than 255, it throws the DataIntegrityViolationException.

Cache:
Redisson is used for caching mechanism.
Update and delete operations are evicted used @CacheEvict annotation and get operations are cached using @Cacheable annotation.

Test:
Tests are not implemented since the lack of time, but I can implement if it is asked.
Good luck!
